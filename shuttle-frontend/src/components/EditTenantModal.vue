<script setup>
// ╔══════════════════════════════════════════════════════════════════════════════╗
// ║  EditTenantModal.vue — Edit existing company/tenant                        ║
// ║  Sends  PUT /api/v1/superadmin/tenants/:id                                ║
// ║  Includes billing/tax info for e-fatura integration                       ║
// ╚══════════════════════════════════════════════════════════════════════════════╝
...
function resetForm() {
  if (props.tenant) {
    ...
    form.planId         = props.tenant.planId || props.tenant.plan?.id || ''
    form.legalName      = props.tenant.legalName || ''
    form.taxOffice      = props.tenant.taxOffice || ''
    form.taxId          = props.tenant.taxId || ''
    form.contactPhone   = props.tenant.contactPhone || ''
    form.billingAddress = props.tenant.billingAddress || ''
  }
  error.value = ''
}
...
async function save() {
  ...
  try {
    const res = await api.put(`/superadmin/tenants/${props.tenant.id}`, {
      name:           form.name,
    ...
    error.value = e.response?.data?.message || e.message || 'Firma güncellenemedi'
  } finally {
          ...
          >
            ...
            <div class="sticky top-0 z-20 flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/95 backdrop-blur-md rounded-t-2xl">
              <div>
                ...
                <p class="text-xs text-slate-500 mt-1 ml-9">Firma ve fatura bilgilerini güncelleyin</p>
              </div>
