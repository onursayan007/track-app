...
// ║  FIX: All Composition API hooks explicitly imported from 'vue'             ║
// ╚══════════════════════════════════════════════════════════════════════════════╝
...
// ─── Tenant List (fetched from API) ─────────────────────────────────────────────
const tenants = ref([])       // [{ id, name }] from /superadmin/tenants
const vehicleModels = ref([]) // [{ id, brand, modelName, photoUrl }]
const isLoading   = ref(false)
const loadError   = ref('')
const saveSuccess = ref('')
const saveError   = ref('')

// ─── Vehicle Fleet Data (fetched from API) ──────────────────────────────────────
const vehicles = ref([])

// ─── Dashboard Stats ────────────────────────────────────────────────────────────
const totalVehicles = computed(() => vehicles.value.length)
const activeDevices = computed(() => vehicles.value.filter(v => v.online).length)
...
const selectedModelPhoto = computed(() => {
  if (!modalForm.vehicleModelId) return ''
  const m = vehicleModels.value.find(vm => vm.id === modalForm.vehicleModelId)
  if (!m?.photoUrl) return ''
  return m.photoUrl.startsWith('http') ? m.photoUrl : `${API_BASE}${m.photoUrl}`
})
...
watch(() => modalForm.dataSource, (newSrc) => {
  ...
  if (newSrc !== 'udp') {
    modalForm.imei = ''
    modalForm.hasM2m = false
    modalForm.m2mNumber = ''
    modalForm.m2mIccid = ''
    modalForm.m2mOperator = ''
    modalForm.m2mDataQuotaMB = ''
  }
})
...
async function saveVehicle() {
  ...
  const hwMap = { arvento: 'ARVENTO', udp: 'UDP', app: 'APP_ONLY' }

  const payload = {
    plate: modalForm.plate,
    vehicleModelId: modalForm.vehicleModelId || null,
    vin:   modalForm.vin,
    hardwareType: hwMap[modalForm.dataSource] || 'APP_ONLY',
    deviceId: modalForm.dataSource === 'arvento'
      ? modalForm.arventoNodeId
      : modalForm.dataSource === 'udp'
        ? modalForm.imei
        : null,
    ...(modalForm.hasM2m && modalForm.dataSource === 'udp' ? {
      m2mNumber: modalForm.m2mNumber || null,
      m2mIccid: modalForm.m2mIccid || null,
      m2mOperator: modalForm.m2mOperator || null,
      m2mDataQuotaMB: modalForm.m2mDataQuotaMB ? parseInt(modalForm.m2mDataQuotaMB) : null,
    } : {}),
  }

  try {
    await api.post(`/superadmin/vehicles?tenantId=${modalForm.tenantId}`, payload)
    saveSuccess.value = 'Araç başarıyla kaydedildi!'
    setTimeout(() => { saveSuccess.value = '' }, 3000)
    closeModal()
    await fetchVehicles()
  } catch (e) {
    saveError.value = e.response?.data?.message || e.message || 'Araç kaydedilemedi'
...
async function fetchVehicles() {
  ...
  loadError.value = ''
  try {
    const res = await api.get('/superadmin/vehicles')
    const list = res.data?.data ?? res.data ?? []
    const hwLabel = { ARVENTO: 'Arvento API', UDP: 'UDP/Teltonika', APP_ONLY: 'Mobil App' }
    vehicles.value = (Array.isArray(list) ? list : []).map(v => {
      const vm = v.vehicleModel
      const modelLabel = vm ? `${vm.brand} ${vm.modelName}` : [v.brand, v.model].filter(Boolean).join(' ') || v.model || '—'
      const photoPath = vm?.photoUrl || ''
      return {
        id:       v.id,
        plate:    v.plate,
        model:    modelLabel,
        photoUrl: photoPath.startsWith('http') ? photoPath : (photoPath ? `${API_BASE}${photoPath}` : ''),
        vin:      v.vin || '',
        tenant:   v.tenant?.name || '—',
        tenantId: v.tenantId,
        sources:  [hwLabel[v.hardwareType] || 'Bilinmiyor'],
        online:   v.status === 'ACTIVE',
        imei:     v.deviceId || '',
      }
    })
  } catch (e) {
    loadError.value = e.response?.data?.message || e.message || 'Araçlar yüklenemedi'
    console.warn('fetchVehicles hata:', e.message)
  } finally {
...
async function fetchTenants() {
  try {
    ...
    if (Array.isArray(list) && list.length) {
      tenants.value = list.map(t => ({ id: t.id, name: t.name }))
    }
  } catch (e) {
...
async function fetchVehicleModels() {
  try {
    ...
    const list = res.data?.data ?? res.data ?? []
    if (Array.isArray(list)) vehicleModels.value = list
  } catch (e) {
    console.warn('fetchVehicleModels hata:', e.message)
  }
}
...
<template>
  <div class="min-h-screen bg-slate-950 text-slate-100 p-6 lg:p-8 space-y-8 font-sans antialiased">
            ...
            >
              ...
              <td class="px-6 py-4">
                <div class="flex items-center gap-3.5">
                  <div class="flex-shrink-0 h-10 w-10 rounded-lg overflow-hidden ring-1 ring-indigo-500/10 group-hover:ring-indigo-500/30 transition">
                    <img v-if="v.photoUrl" :src="v.photoUrl" :alt="v.model" class="h-full w-full object-cover" @error="$event.target.style.display='none'" />
                    <div v-else class="flex items-center justify-center h-full w-full bg-indigo-500/10 text-indigo-400">
    ...
    <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 translate-y-2" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition duration-200 ease-in" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 translate-y-2">
      <div v-if="saveError" class="fixed bottom-6 right-6 z-50 flex items-center gap-2 px-5 py-3 rounded-xl bg-rose-500/20 border border-rose-500/30 text-sm font-medium text-rose-400 shadow-lg backdrop-blur-sm">
                ...
                <section>
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4">
                    <div>
                      ...
                      <input v-model="modalForm.plate" type="text" placeholder="34 ABC 123" class="w-full rounded-lg bg-slate-800/80 border border-slate-700 text-sm text-white px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition" :class="{ 'border-emerald-500/40 bg-emerald-500/5': ocrDone && modalForm.plate }" />
                    </div>
                    <div>
                      <label class="block text-[11px] font-medium text-slate-500 mb-1.5">Araç Modeli</label>
                      <select v-model="modalForm.vehicleModelId" class="w-full rounded-lg bg-slate-800/80 border border-slate-700 text-sm text-white px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition">
                        <option value="">Model seçiniz...</option>
                        <option v-for="vm in vehicleModels" :key="vm.id" :value="vm.id">{{ vm.brand }} — {{ vm.modelName }}</option>
                      </select>
                    </div>
                    <div>
                      <label class="block text-[11px] font-medium text-slate-500 mb-1.5">Şasi No (VIN)</label>
                      <input v-model="modalForm.vin" type="text" placeholder="WDB96340310012345" class="w-full rounded-lg bg-slate-800/80 border border-slate-700 text-sm text-white px-3 py-2.5 font-mono text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition" :class="{ 'border-emerald-500/40 bg-emerald-500/5': ocrDone && modalForm.vin }" />
                    </div>
                  </div>
                  <!-- Vehicle Model Photo Preview -->
                  <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 scale-95" enter-to-class="opacity-100 scale-100" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
                    <div v-if="selectedModelPhoto" class="mt-3 flex items-center gap-3 p-3 rounded-xl bg-slate-800/30 border border-slate-700/50">
                      <img :src="selectedModelPhoto" alt="Model" class="h-14 w-20 object-cover rounded-lg" />
                      <span class="text-xs text-slate-400">Seçilen model fotoğrafı</span>
                    </div>
                  </Transition>
                </section>
                ...
                <section>
                  ...
                  <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 -translate-y-2" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 -translate-y-2">
                    <div v-if="modalForm.dataSource === 'arvento'" class="mt-4">
                      ...
                      <input v-model="modalForm.arventoNodeId" type="text" placeholder="ör: ARV-4582190" class="w-full rounded-lg bg-slate-800/80 border border-slate-700 text-sm text-white px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition" />
                    </div>
                  </Transition>

                  <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 -translate-y-2" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 -translate-y-2">
                    <div v-if="modalForm.dataSource === 'udp'" class="mt-4 space-y-4">
                      <div>
                        <label class="block text-[11px] font-medium text-slate-500 mb-1.5">Cihaz IMEI Numarası</label>
                        <input v-model="modalForm.imei" type="text" placeholder="ör: 359586015829802" class="w-full rounded-lg bg-slate-800/80 border border-slate-700 text-sm text-white px-3 py-2.5 font-mono tracking-wider focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition" />
                      </div>

                      <!-- M2M Hat Toggle -->
                      <div class="flex items-center justify-between rounded-xl bg-slate-800/40 border border-slate-700/50 px-4 py-3">
                        <div>
                          <p class="text-sm font-medium text-slate-300">M2M Hat Var mı?</p>
                          <p class="text-[10px] text-slate-500 mt-0.5">Bu cihaza takılı SIM kart bilgilerini girin</p>
                        </div>
                        <button
                          type="button"
                          @click="modalForm.hasM2m = !modalForm.hasM2m"
                          :class="modalForm.hasM2m ? 'bg-indigo-600' : 'bg-slate-700'"
                          class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
                        >
                          <span :class="modalForm.hasM2m ? 'translate-x-5' : 'translate-x-0'" class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out mt-0.5 ml-0.5"></span>
                        </button>
                      </div>

                      <!-- M2M Fields (expanded) -->
                      <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 -translate-y-2 max-h-0" enter-to-class="opacity-100 translate-y-0 max-h-96" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100 translate-y-0 max-h-96" leave-to-class="opacity-0 -translate-y-2 max-h-0">
                        <div v-if="modalForm.hasM2m" class="space-y-3 overflow-hidden rounded-xl bg-indigo-500/5 border border-indigo-500/20 p-4">
                          <div class="grid grid-cols-2 gap-3">
                            <div>
                              <label class="block text-[11px] font-medium text-slate-400 mb-1.5">M2M Telefon No</label>
                              <input v-model="modalForm.m2mNumber" type="tel" placeholder="05xx xxx xxxx" class="w-full rounded-lg bg-slate-800/80 border border-slate-700 text-sm text-white px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition" />
                            </div>
                            <div>
                              <label class="block text-[11px] font-medium text-slate-400 mb-1.5">ICCID (SIM Seri No)</label>
                              <input v-model="modalForm.m2mIccid" type="text" placeholder="8990011234567890123" class="w-full rounded-lg bg-slate-800/80 border border-slate-700 text-sm text-white px-3 py-2.5 font-mono text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition" />
                            </div>
                          </div>
                          <div class="grid grid-cols-2 gap-3">
                            <div>
                              <label class="block text-[11px] font-medium text-slate-400 mb-1.5">Operatör</label>
                              <select v-model="modalForm.m2mOperator" class="w-full rounded-lg bg-slate-800/80 border border-slate-700 text-sm text-white px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition">
                            ...
                            <div>
                              <label class="block text-[11px] font-medium text-slate-400 mb-1.5">Veri Kotası (MB)</label>
                              <input v-model="modalForm.m2mDataQuotaMB" type="number" placeholder="500" class="w-full rounded-lg bg-slate-800/80 border border-slate-700 text-sm text-white px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 transition" />
                            </div>
                          </div>
                        </div>
                      </Transition>
...
.overflow-y-auto::-webkit-scrollbar-thumb:hover { background: rgba(100, 116, 139, 0.5); }
</style>